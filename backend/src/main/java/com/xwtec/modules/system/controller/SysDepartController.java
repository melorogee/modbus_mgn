...
import com.xwtec.modules.system.model.DepartIdModel;
...
public class SysDepartController {
   ...
   public Result<SysDepart> delete(@RequestParam(name="id",required=true) String id) {
	...
	@RequestMapping(value = "/queryDepartChildList", method = RequestMethod.GET)
	public Result<List<Map<String, Object>>> queryDepartChildList(String orgType){
		Result<List<Map<String, Object>>> result = new Result<>();
		try {
			List<Map<String, Object>> departList = sysDepartService.queryDepartChildList(orgType);
			result.setResult(departList);
			result.setSuccess(true);
		}catch (Exception e){
			result.setSuccess(false);
			log.error("查询下级组织异常",e);
		}
		return result;
	}
}
