package com.xwtec.modules.shiro.authc;
...
public class MyRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
		...
		if(null != sysUser){
			...
			if (!jwtTokenRefresh(token, username, sysUser.getPassword(),JwtUtil.EXPIRE_TIME)) {
	 ...
	 * @return
	 */
	public boolean jwtTokenRefresh(String token, String userName, String passWord,long time) {
		...
		if (oConvertUtils.isNotEmpty(cacheToken)) {
			...
			if (!JwtUtil.verify(token, userName, passWord)) {
				...
				redisUtil.set(CommonConstant.PREFIX_USER_TOKEN + token, newAuthorization);
				 //设置超时时间
				redisUtil.expire(CommonConstant.PREFIX_USER_TOKEN + token, time/1000);
			} else {
